module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}setup`)){
			...
			await channel.send('**Starting CM_logs Setup!**\n*ETA: 30 seconds*').then(msg => {
				response = msg
			})
			ids = ''
			info = {
			...
			if(x){
				if(!guild.channels.cache.find(cc => cc.name === `📢・cisco-announcements`)){
					await guild.channels.create("📢・cisco-announcements", {
							type: 'GUILD_TEXT',
						...
						c.send("This channel has been made so you can follow <#1094390098755330158>, <#1094390098755330158> and don't miss any updated!")
					})
				}
				for(const v in info) {
