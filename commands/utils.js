module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}hide`)){
			...
			let status = await GetResourceKvpString(`CM_logs:${(args[2])}:${args[3]}`)
			if(status === "false" || status === null){
				status = 'true'
			} else {
				status = 'false'
			}
			await SetResourceKvp(`CM_logs:${args[2].toLowerCase()}:${args[3].toLowerCase()}`, status)
...
\`${client.config.prefix}setup\` - Run the CM_logs Setup.
\`${client.config.prefix}uninstall\` - Remove all channels used in CM_logsV3.
