...
CreateThread(function()
    while Config == nil do
        TriggerServerEvent("Cisco:CM_logs:GetConfigSettings") --[[ On client load this will make a request to the server to pull some config data. ]]
        Wait(0)
    end
    while lang == nil do
        if Config ~= nil then
...
RegisterNetEvent("Cisco:CM_logs:SendConfigSettings")
AddEventHandler("Cisco:CM_logs:SendConfigSettings", function(data)
...
RegisterNetEvent('Cisco:CM_logs:ClientCreateScreenshot')
AddEventHandler('Cisco:CM_logs:ClientCreateScreenshot', function(args)
    exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/'..args.url, 'files[]', function(data)
		...
		if resp.attachments then
            ...
            TriggerServerEvent('Cisco:CM_logs:ScreenshotCB', args)
        end
    end)
end)
...
exports('discord', function(msg, player_1, player_2, color, channel) --[[ This is to support the export from v1. ]]
	...
	TriggerServerEvent('Cisco:CM_logs:ClientDiscord', args)
end)
        ...
        elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then --[[ When they player is finished shooring or the timeout has expires we will make the log. ]]
            ...
            if fireCount ~= 0 and timeout == 0 then --[[ When the timer is 0 and the weapon fire count is higher than 0 we will make a log. ]]
                if not ClientTables.WeaponNames[tostring(fireWeapon)] then --[[ Weapon info not found. creating log without weapon info. ]]
                    TriggerServerEvent('Cisco:CM_logs:playerShotWeapon', lang.shooting.undefined)
                    return
                end
                isLoggedWeapon = true
                ...
                if isLoggedWeapon then --[[ Sending the log with weapon info. ]]
                    TriggerServerEvent('Cisco:CM_logs:playerShotWeapon', ClientTables.WeaponNames[tostring(fireWeapon)], fireCount)
                end
                fireCount = 0
            end
        end
...
CreateThread(function()
	...
	while Config.damageLog do --[[ Checking if damage logs is enabled. ]]
		...
		if health > GetEntityHealth(PlayerPedId()) then
			...
			TriggerServerEvent('Cisco:CM_logs:PlayerDamage', math.floor((health - newHealth) / 2))
			health = newHealth
			Wait(1000)
		else
			Wait(1000)
	...
	while Config.deathLog do
		...
		if IsEntityDead(iPed) then
			if not hasRun then
				...
				TriggerServerEvent('Cisco:CM_logs:playerDied', { ['rsn'] = DeathReason, ['kil'] = GetPlayerServerId(killer) })
			end
		else
			Wait(500)
			hasRun = false
