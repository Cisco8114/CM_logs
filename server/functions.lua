...
ServerFunc.CreateLog = function(data)
    ...
    if Config ~= nil then
		if not Config.token or Config.token == "" then
			return print('^1Error:^0 Issue loading config file. Please follow the installation guild on the docs: https://docs.ciscomods.com')
		end
    ...
    if data.screenshot then --[[ this log requires a screenshot to be made so we will transfer to client to grab a screenshot. ]]
        ...
        return TriggerClientEvent('Cisco:CM_logs:ClientCreateScreenshot', data.player_id, data)
    end
    if data.screenshot_2 then --[[ this log requires a second screenshot to be made so we will transfer to client to grab a screenshot. ]]
        ...
        local theFile = json.decode(channelsLoadFile)
        data.url = theFile.imageStore.webhookID.."/"..theFile.imageStore.webhookToken
        return TriggerClientEvent('Cisco:CM_logs:ClientCreateScreenshot', data.player_2_id, data)
    end
            ...
            embeds = {
                {
                    ...
                    footer = {
                        text = "CM_logs V" .. GetResourceMetadata(GetCurrentResourceName(), 'version') .. "  •  Made by Cisco Mods",
                        icon_url = "https://store.ciscomodifications.com/assets/logo.png"
                    },
...
function GetPlayerDetails(src, channel) --[[ Function to grab player details. ]]
    ...
    value = ""
    if Config.playerId and GetResourceKvpString("CM_logs:"..channel:lower()..":playerid") ~= 'true' then
        sid = src
    ...
    if Config.postals and GetResourceKvpString("CM_logs:"..channel:lower()..":postals") ~= 'true' then
        value = value .. "\n`🗺️` **Nearest Postal:** `" .. GetPlayerPostal(src) .. "`"
    end
    if Config.playerHealth and GetResourceKvpString("CM_logs:"..channel:lower()..":health") ~= 'true' then
        value = value .. "\n`❤️` **Health:** `" .. math.floor(GetEntityHealth(GetPlayerPed(src)) / 2) .. "/100`"
    end
    if Config.playerArmor and GetResourceKvpString("CM_logs:"..channel:lower()..":armor") ~= 'true' then
        value = value .. "\n`🛡️` **Armour:** `" .. math.floor(GetPedArmour(GetPlayerPed(src))) .. "/100`"
    end
    if Config.discordId.enabled and GetResourceKvpString("CM_logs:"..channel:lower()..":discordid") ~= 'true' then
    ...
    if Config.ip and GetResourceKvpString("CM_logs:"..channel:lower()..":ip") ~= 'true' then
        value = value .. "\n`🔗` **IP:** ||" .. ids.ip:gsub("ip:", "") .. "||"
    end
    if Config.playerPing and GetResourceKvpString("CM_logs:"..channel:lower()..":ping") ~= 'true' then
        value = value .. "\n`📶` **Ping:** `" .. GetPlayerPing(src) .. "ms`"
    end
    if Config.steamId.enabled and GetResourceKvpString("CM_logs:"..channel:lower()..":steamid") ~= 'true' then
    ...
    if Config.steamUrl and GetResourceKvpString("CM_logs:"..channel:lower()..":steamurl") ~= 'true' then
    ...
    if Config.license.enabled and GetResourceKvpString("CM_logs:"..channel:lower()..":license") ~= 'true' then
