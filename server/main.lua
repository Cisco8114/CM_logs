...
CreateThread(function() --[[ Permissions check for buildin DiscordAcePerms ]]
	...
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_principal') then
		print('^1Error:^0 CM_logs needs to have access to ^2add_principal^0 for the discord permissions to work.')
		add_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_principal') then
		print('^1Error:^0 CM_logs needs to have access to ^2remove_principal^0 for the discord permissions to work.')
		remove_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_ace') then
		print('^1Error:^0 CM_logs needs to have access to ^2add_ace^0 for the discord permissions to work.')
		addd_ace = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_ace') then
		print('^1Error:^0 CM_logs needs to have access to ^2remove_ace^0 for the discord permissions to work.')
		remove_ace = false
	end
	...
	if Config ~= nil then
		if not Config.token then
			print('^1Error: Issue loading config file. Please follow the installation guild on the docs: https://docs.ciscomodifications.com^0')
		end
...
AddEventHandler("playerJoining", function(newID, oldID) --[[ Name Change Logs / Discord Ace Perms. ]]
	...
	local oldName = GetResourceKvpString("CM_logs:nameChane:"..ids.license)
	if oldName == nil then
		SetResourceKvp("CM_logs:nameChane:"..ids.license, GetPlayerName(newID))
	else
...
RegisterNetEvent("Cisco:CM_logs:GetConfigSettings")
AddEventHandler("Cisco:CM_logs:GetConfigSettings", function()
	if Config ~= nil then
		data = {
		...
		TriggerClientEvent("Cisco:CM_logs:SendConfigSettings", source, data)
	end
end)

RegisterServerEvent('Cisco:CM_logs:playerShotWeapon') --[[ Shooting logs. ]]
AddEventHandler('Cisco:CM_logs:playerShotWeapon', function(weapon, count)
...
RegisterServerEvent('Cisco:CM_logs:playerDied')
AddEventHandler('Cisco:CM_logs:playerDied',function(args)
	if args.kil == 0 then
...
RegisterServerEvent('Cisco:CM_logs:PlayerDamage') --[[ Damaghe Logs. ]]
AddEventHandler('Cisco:CM_logs:PlayerDamage', function(args)
...
RegisterServerEvent('Cisco:CM_logs:ClientDiscord')
AddEventHandler('Cisco:CM_logs:ClientDiscord', function(args)
...
RegisterNetEvent("Cisco:CM_logs:ScreenshotCB") --[[ Returning screenshot value. ]]
AddEventHandler("Cisco:CM_logs:ScreenshotCB", function(args)
	ServerFunc.CreateLog(args)
end)

CreateThread(function() --[[ System Messages. ]]
    while true do
        PerformHttpRequest('https://api.ciscomods.com/v1/fivem/cmlogs/systemMsg', function(code, res, headers)
            if code == 200 then
				local rv = json.decode(res)
				if rv.item.id then
					if os.time(os.date("!*t")) - tonumber(rv.item.date) < (7 * 24 * 60 * 60) then
						if GetResourceKvpString('CM_logs:SystemMessage') ~= rv.item.message then
							print('^1CM_logs System Message\n^1--------------------^0\n^2'..rv.item.title..'^0\n'..rv.item.message..'\n^1--------------------^0')
							ServerFunc.CreateLog({ EmbedMessage = '**'..rv.item.title..'**\n'..rv.item.message, channel = 'system'})
						end
					end
					SetResourceKvp("CM_logs:SystemMessageId", ''..rv.item.id..'')
					SetResourceKvp("CM_logs:SystemMessage", ''..rv.item.message..'')
				end
            end
        end, 'GET', nil, {
            ['Token'] = 'CM_logsV3',
			['Last'] = GetResourceKvpString('CM_logs:SystemMessageId')
        })
        Wait(15 * 60 * 1000)
    end
end)
...
-------------------------------------------------------^0]]):format(rv.version, rv.changelog))
				ServerFunc.CreateLog({ EmbedMessage = "**CM_logsV3 Update V"..rv.version.."**\nDownload the latest update of CM_logsV3 here:\nhttps://github.com/cisco/CM_logs/\n\n**Changelog:**\n"..rv.changelog..'\n\n**How to update?**\n1. Download the latest version.\n2. Replace all files with your old once **EXCEPT THE CONFIG** folder.\n3. run the `!jdlogs setup` command again and you\'re done.', channel = 'system'})
			end
		end
	end, 'GET')
end)
